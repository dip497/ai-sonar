...
class FeedbackManager:
    ...
    def __init__(self, feedback_file: str = "feedback.json", memory: Optional[AgentMemory] = None):
        ...
        """
        # Ensure we use an absolute path for the feedback file
        if not os.path.isabs(feedback_file):
            # Store in the same directory as this file
            base_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            self.feedback_file = os.path.join(base_dir, feedback_file)
        else:
    ...
    def process_automated_feedback(self, issue_key: str, fixed_code: str, original_code: str) -> FeedbackItem:
        ...
        if fixed_code == original_code:
            ...
            if not original_lines:
                line_diff_percent = 1.0 if line_diff > 0 else 0.0
            else:
                line_diff_percent = line_diff / len(original_lines)
