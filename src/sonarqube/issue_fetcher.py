...
class SonarQubeIssueFetcher:
    ...
    def fetch_new_issues(self, max_issues=50, days=1):
        ...
        # Add timezone info to avoid issues with SonarQube API
        created_date = datetime.now() - timedelta(days=days)
        try:
            # Try to add timezone info
            created_after = created_date.astimezone().strftime("%Y-%m-%dT%H:%M:%S%z")
        except ValueError:
            # If that fails, use a simpler format without timezone
            created_after = created_date.strftime("%Y-%m-%dT%H:%M:%S+0000")
    ...
    def fetch_issues_since_last_build(self, jenkins_client, max_issues=50):
        ...
        created_date = datetime.fromtimestamp(last_build_timestamp / 1000)
        try:
            # Try to add timezone info
            created_after = created_date.astimezone().strftime("%Y-%m-%dT%H:%M:%S%z")
        except ValueError:
